...
int calc_velocity_pulses(double velocity)
{
	double Pulse_Time = 4248703.506*(velocity*velocity)-301809.6864*(velocity)+6355.748711;
	int returnVal = (int)Pulse_Time;
	printf_P(PSTR("Velocity '%f', converted to %f set Pulse_Time to '%d'\n"),velocity, Pulse_Time, returnVal);
	return returnVal;
}
...
void move_set_time(int time_in_seconds)
{
	...
	float step_distance_float = time_in_seconds/(velocity_delay[0] / 1000);
	int step_distance = (int)step_distance_float;
	printf_P(PSTR("Time '%d' sets step_distance for '%d' \n"), time_in_seconds, step_distance);
	int x,y;
...
void move_set_dist(int Distance_mm)
{
	 ...
	 */
	float Distance_m = ((float)Distance_mm/1000);
	printf_P(PSTR("Distance_mm %d\nDistance_m %f\n"),Distance_mm,Distance_m);
	float step_distance_float = ((float)Distance_m/WHEEL_RADIUS) * STEPS_PER_REV;
	int step_distance = (int)step_distance_float;
