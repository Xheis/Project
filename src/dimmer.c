...
{
    if(step >= DIMMER_MAX_STEP)
   {
	   _step = DIMMER_MAX_STEP;
   }
   else if(step  <= DIMMER_MIN_STEP)   
   {
	   _step = DIMMER_MIN_STEP;
   }
	else _step = step;
}
...
{
	//uint8_t temp = _step +_brightness;
   if((_step +_brightness) >= DIMMER_MAX_BRIGHTNESS)
   {
	  _brightness = DIMMER_MAX_BRIGHTNESS;
   }
...
{
	//uint8_t temp = _step +_brightness;
   if((_step -_brightness) >= DIMMER_MIN_BRIGHTNESS)
   {
	  _brightness = DIMMER_MIN_BRIGHTNESS;
   }
   else _brightness -= _step;
}
